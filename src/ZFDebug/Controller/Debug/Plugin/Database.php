...
class ZFDebug_Controller_Plugin_Debug_Plugin_Database extends ZFDebug_Controller_Plugin_Debug_Plugin implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function getTab()
    {
        ...
        foreach ($this->_db as $adapter) {
            ...
            $adapterInfo[] = count($adapter->getProfiler()->getQueryProfiles()).' in '.round($profiler->getTotalElapsedSecs()*1000, 2).' ms';
        }
    ...
    public function getPanel()
    {
        ...
        foreach ($this->_db as $name => $adapter) {
            if ($profiles = $adapter->getProfiler()->getQueryProfiles()) {
                ...
                foreach ($profiles as $profile) {
                    $html .= '<li><span style="color:#000">['.round($profile->getElapsedSecs()*1000, 2).' ms]</span> '
        ...
        $html = preg_replace('#(DESCRIBE|SELECT|FROM|WHERE|GROUP BY|ORDER BY|UNION|LEFT JOIN|INNER JOIN)#', '<b style="color:#000">$1</b>', $html);
        $html = preg_replace('#\s(AND|OR)\s#', ' <b style="color:green">$1</b> ', $html);
